"""Shared demo-car catalog and seeding logic — used by both the CLI script
(scripts/seed_cars.py, for local/manual use) and the admin "Seed Demo Cars"
button (app/routes/storefront_admin.py, for seeding a live deployment where
you don't have shell/SSH access, such as Render's free tier)."""
...
def seed_demo_cars():
    """Adds any demo cars not already present (by title) to the database.
    Safe to call repeatedly — matches by title so it never creates
    duplicates. Returns (added_count, skipped_count)."""
    from app import db
    from app.models.product import Product
    from app.models.car import CarListing
    from app.utils.codes import unique_slug, next_listing_code
